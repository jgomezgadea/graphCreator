...
class GraphNode{
	...
	protected:
        ...
        //bool reserveNodeServiceServerCb(robotnik_fms_msgs::BlockNode::Request& request, robotnik_fms_msgs::BlockNode::Response& response); // Callback for getting the node info
...
void GraphNode::push_al(robotnik_msgs::alarmsmonitor *Alarms,std::string type, std::string group,int line , const char *calledby, std::string text,bool bPrint,bool bHMI){
    ...
    if (strlen(calledby)>0) grp=grp+"::"+std::string(calledby);
    if (line>0) grp=grp+"::"+std::to_string(line);
...
void GraphNode::rosReadParams(){
    ...
    if (dGraphFreq_<=0) dGraphFreq_=1.0;
...
void GraphNode::rosPublish(){
	...
	msg.magnets = graph_route->GetMagnets();
    ...
    if (route.size()>0) {
        ...
        for (int i=0;i<route.size();i++){
            ...
            if (nod->bDoorOpen) {
                ...
                marker_array_msg.markers[i].color.b = 255;
            ...
            if (nod->bDoorClose) {
                ...
                shape = visualization_msgs::Marker::CYLINDER;
                marker_array_msg.markers[i].color.r = 0.0;
                marker_array_msg.markers[i].color.g = 255;
    ...
    if (graph_route->dijkstraGraph->vNodes.size()>0) {
        ...
        for(int i=0;i<graph_route->dijkstraGraph->vNodes.size();i++){
            ...
            */
            ...
            if (nod.bLoad)   fR=0.5;
            if (nod.bUnload) fG=0.5;
            if (nod.bCharge) fB=0.5;
            ...
            std::string txt;
            if (sZones.length()>1) txt=sZones+":";
            else txt="";
            if (nod.bDoorOpen) txt=txt+"DO";
            if (nod.bDoorClose) txt=txt+"DC";
            if (nod.bEnterShower) txt=txt+"ESH:"+nod.sShowerFrom+":"+nod.sShowerDoor;
            if (nod.bLeaveShower) txt=txt+"LSH:"+nod.sShowerFrom+":"+nod.sShowerDoor;

            if (nod.bLoad) txt=txt+"L";
            if (nod.bUnload) txt=txt+"UL";
			if (nod.bPrePick) txt=txt+"PRL";
			if (nod.bPrePlace) txt=txt+"PRUL";
			if (nod.bPostPick) txt=txt+"POL";
			if (nod.bPostPlace) txt=txt+"POUL";
            if (nod.bCharge) txt=txt+"C";

            if (nod.bDoNothing) txt=txt+"DoNothing";
            ...
            if (nod.iResRobot>=0) txt=txt+" Res:"+std::to_string(nod.iResRobot);
            if (nod.iRobot>=0) txt=txt+" R:"+std::to_string(nod.iRobot);
            MText.text = txt;
...
void GraphNode::CopyNodeToMessage(Node *Or,robotnik_fms_msgs::NodeInfo *Dest){
    ...
    Dest->stop=Or->bStop;
    Dest->acoustic_signal=Or->bAcoustic;
    Dest->charge=Or->bCharge;
    //Dest->blocked=Or->bBlocked;
    Dest->charge=Or->bCharge;

    Dest->floor=Or->iFloor;
    Dest->id=Or->iNode;
    Dest->laser_range=Or->iLaser_Range;
    Dest->load=Or->bLoad;
    ...
    Dest->unload=Or->bUnload;
    Dest->prepick=Or->bPrePick;
    Dest->preplace=Or->bPrePlace;
    Dest->postpick=Or->bPostPick;
    Dest->postplace=Or->bPostPlace;
    Dest->robot=Or->iRobot;
    Dest->robot_reserved=Or->iResRobot;
    Dest->doorclose=Or->bDoorClose;
    Dest->dooropen=Or->bDoorOpen;
    Dest->entershower=Or->bEnterShower;
    Dest->leaveshower=Or->bLeaveShower;
    Dest->showerfrom=Or->sShowerFrom;
    Dest->showerdoor=Or->sShowerDoor;

    Dest->door_id=Or->iDoorID;
    Dest->name=std::string(Or->cName);


    Dest->leavemagnetic=Or->bLeaveMagnetic;
    Dest->leavemagnetic_turn_direction=Or->sLeaveMagneticTurn;

    Dest->findmagnetic=Or->bFindMagnetic;
    Dest->findmagnetic_distance=Or->fFindMagneticDistance;
    Dest->findmagnetic_turn_direction=Or->sFindMagneticTurnDirection;

    Dest->magneticload=Or->bMagneticLoad;
    Dest->magneticunload=Or->bMagneticUnload;

    Dest->magnetic_load_unload_allow_markers=Or->bMagneticLoadUnloadAllowMarkers;
    Dest->magnetic_load_unload_cart_id=Or->sMagneticLoadUnloadCartID;
    Dest->magnetic_load_unload_cart_position=Or->iMagneticLoadUnloadCartPosition;
    Dest->magnetic_load_unload_lane=Or->iMagneticLoadUnloadLane;
    Dest->magnetic_load_unload_turn_direction=Or->sMagneticLoadUnloadTurn;

    Dest->elevator=Or->bElevator;
    Dest->elevator_id=Or->iElevatorID;
    Dest->elevator_open_door=Or->bElevatorOpenDoor;
    Dest->elevator_close_door=Or->bElevatorCloseDoor;
    Dest->elevator_get_control=Or->bElevatorGetControl;
    Dest->elevator_leave_control=Or->bElevatorLeaveControl;
    Dest->elevator_floor=Or->iElevatorFloor;

    Dest->do_nothing=Or->bDoNothing;

    Dest->switchmap=Or->bSwitchMap;

    Dest->switchmap_name=Or->sSwitchMapName;
    Dest->switchmap_x=Or->fSwitchMapX;
    Dest->switchmap_y=Or->fSwitchMapY;
    Dest->switchmap_theta=Or->fSwitchMapTheta;

    Dest->move=Or->bMove;
    Dest->move_goal.x=Or->fMoveGoalX;
    Dest->move_goal.y=Or->fMoveGoalY;
    Dest->move_goal.theta=Or->fMoveGoalTheta;
    Dest->move_maximum_velocity.linear.x=Or->fMoveMaxVelX;
    Dest->move_maximum_velocity.linear.y=Or->fMoveMaxVelY;
    Dest->move_maximum_velocity.angular.z=Or->fMoveMaxVelTheta;

    Dest->leave_lift=Or->bLeave_Lift;
    Dest->enter_lift=Or->bEnter_Lift;
    Dest->lift_floor=Or->sLift_Floor;
...
void GraphNode::CopyMessageToNode(robotnik_fms_msgs::NodeInfo *Or,Node *Dest){
    ...
    Dest->bStop=Or->stop;
    Dest->bAcoustic=Or->acoustic_signal;
    Dest->bCharge=Or->charge;

    //Dest->bBlocked=Or->blocked;
    Dest->bCharge=Or->charge;

    Dest->iFloor=Or->floor;
    Dest->iNode=Or->id;
    Dest->iLaser_Range=Or->laser_range;
    Dest->bLoad=Or->load;
    ...
    Dest->bUnload=Or->unload;
    Dest->bPrePick=Or->prepick;
    Dest->bPrePlace=Or->preplace;
    Dest->bPostPick=Or->postpick;
    Dest->bPostPlace=Or->postplace;
    Dest->iRobot=Or->robot;
    Dest->iResRobot=Or->robot_reserved;
    Dest->bDoorClose=Or->doorclose;
    Dest->bDoorOpen=Or->dooropen;
    Dest->bEnterShower=Or->entershower;
    Dest->bLeaveShower=Or->leaveshower;
    Dest->sShowerFrom=Or->showerfrom;
    Dest->sShowerDoor=Or->showerdoor;

    Dest->iDoorID=Or->door_id;
    strcpy(Dest->cName,Or->name.c_str());


    Dest->bLeaveMagnetic=Or->leavemagnetic;
    Dest->sLeaveMagneticTurn=Or->leavemagnetic_turn_direction;

    Dest->bFindMagnetic=Or->findmagnetic;
    Dest->fFindMagneticDistance=Or->findmagnetic_distance;
    Dest->sFindMagneticTurnDirection=Or->findmagnetic_turn_direction;

    Dest->bMagneticLoad=Or->magneticload;
    Dest->bMagneticUnload=Or->magneticunload;

    Dest->bMagneticLoadUnloadAllowMarkers=Or->magnetic_load_unload_allow_markers;
    Dest->sMagneticLoadUnloadCartID=Or->magnetic_load_unload_cart_id;
    Dest->iMagneticLoadUnloadCartPosition=Or->magnetic_load_unload_cart_position;
    Dest->iMagneticLoadUnloadLane=Or->magnetic_load_unload_lane;
    Dest->sMagneticLoadUnloadTurn=Or->magnetic_load_unload_turn_direction;

    Dest->bElevator=Or->elevator;
    Dest->iElevatorID=Or->elevator_id;
    Dest->bElevatorOpenDoor=Or->elevator_open_door;
    Dest->bElevatorCloseDoor=Or->elevator_close_door;
    Dest->bElevatorGetControl=Or->elevator_get_control;
    Dest->bElevatorLeaveControl=Or->elevator_leave_control;
    Dest->iElevatorFloor=Or->elevator_floor;

    Dest->bDoNothing=Or->do_nothing;

    Dest->bSwitchMap=Or->switchmap;

    Dest->sSwitchMapName=Or->switchmap_name;
    Dest->fSwitchMapX=Or->switchmap_x;
    Dest->fSwitchMapY=Or->switchmap_y;
    Dest->fSwitchMapTheta=Or->switchmap_theta;

    Dest->bMove=Or->move;
    Dest->fMoveGoalX=Or->move_goal.x;
    Dest->fMoveGoalY=Or->move_goal.y;
    Dest->fMoveGoalTheta=Or->move_goal.theta;
    Dest->fMoveMaxVelX=Or->move_maximum_velocity.linear.x;
    Dest->fMoveMaxVelY=Or->move_maximum_velocity.linear.y;
    Dest->fMoveMaxVelTheta=Or->move_maximum_velocity.angular.z;

    Dest->bLeave_Lift=Or->leave_lift;
    Dest->bEnter_Lift=Or->enter_lift;
    Dest->sLift_Floor=Or->lift_floor;
}
...
bool GraphNode::routeServiceServerCb(robotnik_fms_msgs::GetRoute::Request& request, robotnik_fms_msgs::GetRoute::Response& response){
    ...
    if (request.from_node==request.to_node){
        ...
        if(graph_route->GetRoute(request.from_node, request.to_node, &v_nodes, &v_magnets, &v_velocities) != 0){
        ...
        for(int i = 0; i < (int)v_nodes.size(); i++){
            ...
            if (nod.stop) nodes=nodes + "P,";
            else nodes=nodes + ",";
            response.nodes.push_back(nod);
            //response.nodes.insert(response.nodes.begin(),nod);
        }
        for(int i = 0; i < (int)v_magnets.size(); i++){
            response.magnets.push_back(v_magnets[i].iNumber);
            pose.x = v_magnets[i].dX;
            pose.y = v_magnets[i].dY;
            pose.theta = 0.0;
            response.magnet_positions.push_back(pose);
        }
...
bool GraphNode::BlockNode(bool bBlock,int idNode, int iRobot,string *msg){
    ...
    if (bBlock){
        ...
        //    node_info->bBlocked=true;
            ...
            if (bDemo_) graph_route->ReserveNode(iRobot,idNode);
            return true;
        }
    } else {
        //if (node_info->bBlocked){
...
// Callback handler for the service server
bool GraphNode::reserveNodeServiceServerCb(robotnik_fms_msgs::BlockNode::Request& request, robotnik_fms_msgs::BlockNode::Response& response){
    std::string msg="BlockNodeService Robot:"+ std::to_string(request.robot_id) +" to reserve: " + std::to_string(request.reserve) + " Nodes:";
    std::string nodes="";
    for (int i=0;i<request.node_id.size();i++) nodes = nodes + std::to_string(request.node_id[i]) + ",";
    msg=msg+nodes;
    ROS_INFO("GraphNode: reserveNodeServiceServerCb: Calling to RESERVE:%d, NODES:%s, ROBOT:%d",request.reserve,nodes.c_str(),request.robot_id);
    pthread_mutex_lock(&mutexGraph);

    for(int i=0; i<request.node_id.size();i++){
        std::string msg_local;
        bool bret=CheckNode(request.reserve,request.node_id[i],request.robot_id,&msg_local);
        if (!bret){
            response.ret = false;
    ...
    for(int i=0; i<request.node_id.size();i++){
        std::string msg_local;
        bool bret=ReserveNode(request.reserve,request.node_id[i],request.robot_id,&msg_local);
        if (!bret){
            response.ret = false;
            response.msg = msg + " Not Possible:" + msg_local;
            PUSH_EVENT(msg);
...
bool GraphNode::blockNodeServiceServerCb(robotnik_fms_msgs::BlockNode::Request& request, robotnik_fms_msgs::BlockNode::Response& response){
    std::string msg="BlockNodeService Robot:"+ std::to_string(request.robot_id) +
            ...
            " to reserve:" + std::to_string(request.reserve)*/ + " Nodes:";
    std::string nodes="";
    for (int i=0;i<request.node_id.size();i++) nodes = nodes + std::to_string(request.node_id[i]) + ",";
    msg=msg+nodes;
